"""
Script om de categorisatie van het 2e meest recente artikel te repareren met RouteLLM.
"""
import sys
import os
...
def fix_second_article():
    """Repareer de categorisatie van het 2e meest recente artikel."""
    print("=" * 80)
    print("REPARATIE 2E MEEST RECENTE ARTIKEL CATEGORISATIE")
    print("=" * 80)
    
    supabase = get_supabase_client()
    
    try:
        # Haal de 2 meest recente artikelen op
        print("\n[INFO] Haal 2 meest recente artikelen op...")
        articles = supabase.get_articles(limit=2)
        
        if len(articles) < 2:
            print(f"[WARN] Minder dan 2 artikelen gevonden (alleen {len(articles)})")
            if len(articles) == 1:
                print("[INFO] Repareer het enige beschikbare artikel...")
                article = articles[0]
            else:
                print("[WARN] Geen artikelen gevonden")
                return
        else:
            # Neem het 2e artikel (index 1)
            article = articles[1]
