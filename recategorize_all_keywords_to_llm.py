"""
Script om ALLE artikelen die met Keywords zijn gecategoriseerd opnieuw te categoriseren met RouteLLM.
"""
import sys
import os
...
def recategorize_all_keyword_articles_with_routellm():
    """Recategoriseer ALLE artikelen die met Keywords zijn gecategoriseerd met RouteLLM."""
    print("=" * 60)
    print("RECATEGORISATIE - ALLE KEYWORDS NAAR ROUTELLM")
    print("=" * 60)
    ...
    print(f"[INFO] Haal ALLE artikelen op die met Keywords zijn gecategoriseerd...\n")
    
    supabase = get_supabase_client()
    
    try:
        # Get ALL articles categorized with Keywords (or Keywords-Fallback, Keywords-Error, LLM-Failed)
        # Limit to last 72 hours only
        from datetime import datetime, timedelta
        cutoff_date = datetime.now(pytz.UTC) - timedelta(hours=72)
        cutoff_date_str = cutoff_date.isoformat()
        
        response = supabase.client.table('articles').select('*').in_('categorization_llm', ['Keywords', 'Keywords-Fallback', 'Keywords-Error', 'LLM-Failed']).gte('published_at', cutoff_date_str).order('published_at', desc=True).execute()
        
        articles = response.data if hasattr(response, 'data') else []
        
        if not articles:
            print("[INFO] Geen artikelen gevonden die met Keywords/LLM-Failed zijn gecategoriseerd (laatste 72 uur).")
            return
        ...
        for i, article in enumerate(articles, 1):
            try:
                ...
                print(f"  [{i}/{len(articles)}] Categoriseren: {title[:60]}...")
                ...
                try:
                    ...
                    if result and isinstance(result, dict):
                        ...
                        if main_category or categories:
                            ...
                            print(f"       [OK] {category_display} ({llm})")
                        else:
...
if __name__ == "__main__":
    recategorize_all_keyword_articles_with_routellm()
